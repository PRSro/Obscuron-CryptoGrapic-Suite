...
void autokey(const std::string &a, const std::string &key, std::string &out, bool encrypt) {
    std::string k = key + a;  // extend key with message
    int ki = 0;
    int sign = encrypt ? 1 : -1;
    out = "";
    for (char c : a) {
...
// AFFINE
// encrypt: E(x) = (a*x + b) mod 26 a must be coprime to 26
// decrypt: D(x) = a_inv*(x - b) mod 26
// extended euclidean to find modular inverse of a mod m
// returns -1 if inverse doesn't exist (a not coprime to m)
...
void build_adfgvx_grid(const std::string &key, char grid[6][6]) {
    // 6x6 grid, fits A-Z + 0-9 = 36 chars
    bool used[36] = {};
    std::string sequence;
    // dedup key chars first
    for (char c : key) {
        int idx = -1;
        if (c >= 'A' && c <= 'Z') idx = c - 'A';
...
void build_trifid_grid(const std::string &key, char grid[3][3][3]) {
    ...
    if (!used[26]) sequence += '+'; // filler always at end if not in key
...
void trifid(const std::string &a, std::string &out, const std::string &key, int period, bool encrypt) {
    ...
    for (int start = 0; start < n; start += period) {
        ...
        if (end > n) end = n;
        int blen = end - start;
        int layers[blen], rows[blen], cols[blen];
